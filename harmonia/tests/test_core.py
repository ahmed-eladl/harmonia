...
# ===================================================================
# Version stripping (local specifier +cu128)
# ===================================================================

class TestVersionStripping:
    def test_torch_with_cuda_suffix(self):
        """torch==2.9.1+cu128 should match 2.9.1 in database."""
        env = _env(
            cuda_info=CUDAInfo(smi_version="13.0", driver_version="580.00"),
            gpus=[GPUInfo(name="L4")],
            installed_packages={
                "torch": "2.9.1+cu128",
        ...
        result = check(env)
        # Should NOT warn about unknown torch version
        assert not any(
            i.package == "torch" and "not in our database" in i.message
            for i in result.issues
        )

    def test_companion_with_cuda_suffix(self):
        """torchaudio==2.9.1+cu128 should match expected 2.9.1."""
        env = _env(
            cuda_info=CUDAInfo(smi_version="13.0", driver_version="580.00"),
            gpus=[GPUInfo(name="L4")],
            installed_packages={
        ...
        result = check(env)
        # Should NOT flag companions as wrong
        assert not any(
            i.package in ("torchaudio", "torchvision") and i.severity == Severity.ERROR
            for i in result.issues
        )

    def test_wrong_companion_still_caught(self):
        """torchaudio==2.5.0+cu128 should still fail against torch 2.9.1."""
        env = _env(
            cuda_info=CUDAInfo(smi_version="13.0", driver_version="580.00"),
            gpus=[GPUInfo(name="L4")],
            installed_packages={
        ...
        result = check(env)
        assert any(
            i.package == "torchaudio" and i.severity == Severity.ERROR
            for i in result.issues
        )


# ===================================================================
# New torch versions (2.6+)
# ===================================================================

class TestNewTorchVersions:
    def test_torch_291_in_db(self, pytorch_db):
        info = get_version_info(pytorch_db, "2.9.1")
        assert info is not None
        assert "12.8" in info["cuda"]
        assert "13.0" in info["cuda"]

    def test_torch_260_in_db(self, pytorch_db):
        info = get_version_info(pytorch_db, "2.6.0")
        assert info is not None

    def test_torch_2100_in_db(self, pytorch_db):
        info = get_version_info(pytorch_db, "2.10.0")
        assert info is not None
        assert info["companions"]["torchvision"] == "0.25.0"
...
class TestNewTransformersVersions:
    def test_transformers_4523_in_db(self, transformers_db):
        info = get_transformers_info(transformers_db, "4.52.3")
        assert info is not None

    def test_transformers_4576_in_db(self, transformers_db):
        info = get_transformers_info(transformers_db, "4.57.6")
        assert info is not None
...
class TestCUDA13:
    def test_driver_580_supports_cuda_13(self):
        """Driver 580 should support CUDA 13.0 — no false error."""
        env = _env(
            cuda_info=CUDAInfo(smi_version="13.0", nvcc_version="13.0",
                              torch_cuda_version="12.8", driver_version="580.126.09"),
            gpus=[GPUInfo(name="NVIDIA L4", memory_total_mb=23034)],
            installed_packages={
                "torch": "2.9.1+cu128",
                "torchaudio": "2.9.1+cu128",
                "torchvision": "0.24.1",
        ...
        result = check(env)
        driver_errors = [i for i in result.issues
                         if i.package == "driver" and i.severity == Severity.ERROR]
